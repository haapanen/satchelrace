...
static float CG_DrawTimer( float y ) {
	...
	// sta acqu-sdk (issue 2): CHRUKER: b018 - Respawn timer shouldn't be shown in spectator mode
	...
	//						va("^F%d%s", CG_CalculateReinfTime( qfalse ), ((cgs.timelimit <= 0.0f) ? "" : " ")) : "";
	// end acqu-sdk (issue 2): CHRUKER: b018
...
static void CG_DrawUpperRight( void ) {
	...
	// sta acqu-sdk (issue 2): CHRUKER: b008
	//if( !cg_drawFireteamOverlay.integer ) {
	//	return;
	//}
	// end acqu-sdk (issue 2): CHRUKER: b008

	y = 20 + 100 + 32;

	// sta acqu-sdk (issue 2): CHRUKER: b008  - Round timer and respawn timer gone when cg_drawFireteamOverlay
	if( cg_drawFireteamOverlay.integer ) {
...
void CG_AddLagometerSnapshotInfo( snapshot_t *snap ) {
	...
	// sta acqu-sdk (issue 2): CHRUKER: b093 - Lagometer ping not correct during demo playback
	if (cg.demoPlayback) {
		snap->ping = (snap->serverTime - snap->ps.commandTime) - 50;
	}
	// end acqu-sdk (issue 2): CHRUKER: b093
