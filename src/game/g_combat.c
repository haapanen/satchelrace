...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	qboolean	teamkilled = ( OnSameTeam(self, attacker) || InflictorEntOnSameTeam(self, inflictor) );
...
//		}
    ...
    {
        if( mod == MOD_SATCHEL && 
            inflictor->parent->client && 
            inflictor->parent->client->powerups[PW_SATCHELBOOST] > 0 )
        {
            knockback = knockback * sr_pw_satchelBoostKnockback.value;
        }

        if( mod == MOD_SATCHEL &&
            inflictor->parent->client &&
            inflictor->parent->client->powerups[PW_SATCHELUNBOOST] > 0 )
        {
            knockback = knockback * sr_pw_satchelUnboostKnockback.value;
        }
...
// 	}
	...
	if ( !(dflags & DAMAGE_NO_PROTECTION) ) {
		...
		// sta acqu-sdk (issue 22): improved teamkill/-damage detection
		if ( targ != attacker && ( OnSameTeam(targ, attacker) ||  InflictorEntOnSameTeam(targ, inflictor) ) ) {
